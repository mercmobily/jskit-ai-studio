...
- Target-readiness rule: after Studio Setup Doctor passes and before any app inspection, Studio runs Adapter Setup Doctor in `/setup?tab=adapter-setup` to prove target identity, filesystem access, Git state, and GitHub control capability without reading app metadata.
- App-setup rule: after Adapter Setup Doctor passes and before `/home`, Studio runs Project Setup Doctor in `/setup?tab=project-setup` to make the target root a doctor-ready JSKIT app without duplicating `npx jskit app verify`.
...
- Mandatory Studio Setup runtime: a reliable, Studio-managed environment able to run MySQL, Node 22, npm, git, GitHub CLI, and Codex. The target direction is container-provided runtimes rather than relying on whatever happens to be installed on the host.
...
| --- | --- | --- | --- |
| StudioSetupEnvironment | Machine-level runtime readiness for Studio | public | Checks Docker/runtime ability to provide MySQL capability, Node 22, npm, git, GH auth, and Codex auth before project work begins. |
...
| ProjectSetupReadiness | Sequential setup state for the target app | public | Derived from filesystem, Git/GitHub remote state, JSKIT scaffold markers, local dependencies, runtime service needs, and `npx jskit app verify`. |
...
- Current-app npm script API: `@local/current-app` owns `GET /api/studio/current-app/npm-scripts`, `PUT /api/studio/current-app/npm-scripts/starred`, `DELETE /api/studio/current-app/npm-scripts/starred`, `POST /api/studio/current-app/npm-script-terminal`, `DELETE /api/studio/current-app/npm-script-terminal/:terminalSessionId`, and the websocket at `/api/studio/current-app/npm-script-terminal/:terminalSessionId/ws`. All are local-only Studio routes and validate script names against the current target `package.json`.
- Starred npm script config: missing `.jskit/config/starred_npm_scripts` means the code-owned default shortcuts `jskit:update`, `devlinks`, `build`, `server`, `verify` filtered to scripts that exist. A present blank file means no starred scripts. Saving writes one script name per line in chosen order; reset deletes the file. Newly starred scripts append to the starred order and render before other scripts on `/home/npm-scripts`.
- NPM script terminal: script runs are restricted to `npm run <scriptName>` in the Docker toolchain, reuse the shared xterm/websocket message shape, and cap the UI to one fullscreen visible terminal at a time. Unlike app-test terminals, npm script terminals do not publish app ports or synthesize an `appUrl`; they are for running package scripts, not for hosting a reachable preview server.
...
- MySQL boundary: Studio Setup Doctor must not ask for or create the controlled app's database name. App-specific database names and schemas are decided later from the current app; Studio Setup only proves that the managed MySQL container works and has DDL rights.
...
- Verification seam: runtime Git-ready checks remain toolchain-owned, but service-level tests may inject a read-only Git command runner for linked-worktree readiness checks when the verification environment cannot run the Docker toolchain.
...
| --- | --- | --- | --- | --- |
| Studio Setup Doctor | Block Studio behind machine runtime checks for Docker, MySQL capability, Node 22, npm, git, GH, and Codex. | Generated server package plus gated shell-web UI | Base scaffold | `/setup?tab=studio-setup` shows Studio Setup status, all checks return structured pass/fail data, project UI is unavailable until every required check passes, and tests/Playwright verify the blocked Studio Setup screen. |
...
| Project Setup Doctor | Sequentially make the target root a doctor-ready JSKIT app. | Generated-style server package plus gated shell-web UI | Adapter Setup Doctor | `/setup?tab=project-setup` shows sequential setup stages, hard-stops unsafe Git/filesystem states, repairs safe missing setup through terminal actions, and only passes after JSKIT doctor passes. |
